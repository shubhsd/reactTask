     ...
     _onActionHandeller=()=>{
        ...
        axios.post(url,dataToSend)
            .then(async (response) => {
                ...
                await swal({title:"Signup successfully",text:"",timer:1000, icon: 'success'})
            }).catch((error) => {
                console.log(error,'sdshgdfysdgsg');
                swal({title:this.state.isSignup?"Email already exists":"invalid credentials",text:"",timer:1000, icon: 'error'})
            });
        ...
        return (
            ...
            <div className="confirm-wrapper">
                <div className="content-sect">
                    <h2 className="text-center">{this.state.isSignup?"Sign Up":"Sign In"}</h2>
                    <hr />
                </div>
                <div className="content-sect">
                    <div>
                        {this.state.isSignup&&<div className="form-group">
                        ...
                        <div className="form-group">
                            <input type="button" className="form-control custom-button" value={this.state.isSignup?"Sign Up":"Sign In"} id="btnVerify" onClick={this._onActionHandeller}/>
                        </div>
                    </div>
                </div>
                <div className="content-sect">
